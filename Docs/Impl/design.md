...
球（半径 R・外向き法線）では n_i − n̄ ≈ (q_i − p)/R となり H = 1/R で厳密。メッシュの**描画用法線**を使うため、スムースシェーディングの低ポリゴンメッシュでもポリゴンの面が出ず、シェーディング通りの滑らかな曲率になる。ハードエッジ（法線分割）はエッジを挟んで法線がジャンプするため、幅 r のハイライトとして検出される。
